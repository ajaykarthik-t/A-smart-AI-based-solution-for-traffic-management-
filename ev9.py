...
while running:
    ...
    # Draw current timer display
    state_text = "RED" if traffic_light.state == LIGHT_RED else "YELLOW" if traffic_light.state == LIGHT_YELLOW else "GREEN"
    timer_display = font.render(f"Current Light: {state_text} - {traffic_light.seconds_left}s", True, BLACK)
    screen.blit(timer_display, (WIDTH//2 - 150, 60))
    
    font = pygame.font.SysFont(None, 24)
    red_text = font.render("RED: Stop (60s)", True, RED)
    screen.blit(red_text, (20, 70))
    
    yellow_text = font.render("YELLOW: Wait (5s)", True, (150, 150, 0))
    screen.blit(yellow_text, (20, 100))
    
    green_text = font.render("GREEN: Go (60s)", True, GREEN)
